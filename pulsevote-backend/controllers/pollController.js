...
function hasOrganisationRole(user, organisationId, roleName) {
  return user.roles.some(
    (role) =>
      role.role === roleName &&
      role.organisationId?.toString() === organisationId.toString()
  );
}
  ...
  try {
    ...
    if (!hasOrganisationRole(user, poll.organisationId, "user")) {
      return res.status(403).json({
        message: "Only organisation users may vote in this poll"
      });
    }
...
exports.getPollResults = async (req, res) => {
  try {
    ...
    const isMember = user.roles.some(
      (role) =>
        role.organisationId?.toString() ===
        poll.organisationId.toString()
    );
...
exports.getOrgPolls = async (req, res) => {
  try {
    ...
    const isMember = user.roles.some(
      (role) =>
        role.organisationId?.toString() === organisationId
    );
...
async function setPollStatus(req, res, status) {
  try {
    ...
    const isManager = hasOrganisationRole(
      user,
      poll.organisationId,
      "manager"
    );
...
exports.closePoll = (req, res) =>
  setPollStatus(req, res, "closed");

exports.openPoll = (req, res) =>
  setPollStatus(req, res, "open");